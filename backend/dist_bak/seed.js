...
const user_subscription_entity_2 = require("./user-subscription/user-subscription.entity");
async function runSeed(dataSource) {
    ...
    const nodeRepository = dataSource.getRepository(node_entity_1.Node);
    const vpnConfigRepository = dataSource.getRepository(vpn_configuration_entity_1.VPNConfiguration);
    const orderRepository = dataSource.getRepository(order_entity_1.Order);
    const userSubscriptionRepository = dataSource.getRepository(user_subscription_entity_1.UserSubscription);
    const connectionLogRepository = dataSource.getRepository(connection_log_entity_1.ConnectionLog);
    const systemLogRepository = dataSource.getRepository(system_log_entity_1.SystemLog);
    ...
    try {
        ...
        await orderRepository.save([
            {
                ...
                subscriptionPlanId: 'plan-monthly',
                amount: 19.99,
                paymentMethod: order_entity_1.OrderPaymentMethod.ALIPAY,
            ...
            },
            {
                ...
                amount: 49.99,
                paymentMethod: order_entity_1.OrderPaymentMethod.WECHAT_PAY,
                status: order_entity_1.OrderStatus.COMPLETED,
                payUrl: 'pay-url-2',
        ...
        await userSubscriptionRepository.save([
            ...
            },
            {
                ...
                subscriptionPlanId: 'plan-monthly',
                totalCost: 19.99,
                status: user_subscription_entity_2.SubscriptionStatus.EXPIRED,
                endDate: '2026-05-11',
                trafficUsed: 5120,
                trafficLimit: 10240
            },
        ...
        await systemLogRepository.save([
            {
                level: system_log_entity_1.LogLevel.INFO,
                message: 'User login successful',
                source: system_log_entity_1.LogSource.SYSTEM,
                userId: 'user-1',
                ipAddress: '192.168.1.100'
            },
            {
                level: system_log_entity_1.LogLevel.WARNING,
            ...
            {
                level: system_log_entity_1.LogLevel.ERROR,
                message: 'Payment verification failed',
                source: system_log_entity_1.LogSource.PAYMENT_SERVICE,
                userId: null,
            ...
            {
                level: system_log_entity_1.LogLevel.DEBUG,
...
async function checkIfDataExists(dataSource) {
    const tablesToCheck = [
        'subscription_plans',
        'users',
        'nodes',
    ...
    for (const table of tablesToCheck) {
        const result = await dataSource.query(`SELECT EXISTS (SELECT 1 FROM ${table} WHERE 1=1) as exists`);
